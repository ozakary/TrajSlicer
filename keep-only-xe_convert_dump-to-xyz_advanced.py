...
def convert_lammps_to_xyz(input_file, output_file, filter_type=None, sample_rate=1, 
                     atom_labels=None, start_frame=None, end_frame=None):
    """
                ...
                if frame_lines and "ITEM: TIMESTEP" in frame_lines[0]:
                    ...
                    if start_frame <= current_frame <= end_frame:
                        ...
                        if (current_frame - start_frame) % sample_rate == 0:
                            convert_frame_to_xyz(frame_lines, outfile, filter_type, atom_labels)
                            frames_written += 1
                        pbar.update(1)
...
def convert_frame_to_xyz(frame_lines, outfile, filter_type=None, atom_labels=None):
    """
    ...
    # We need positions (x, y, z) and type
    ...
    for line in atom_lines:
        ...
        if len(parts) >= 2:
            try:
                ...
                # Check if we should keep this atom type
                if filter_type is None or atom_type in filter_type:
                    # Get element label based on atom type
                    ...
                    xyz_atom_lines.append(f"{element} {x} {y} {z}")
    ...
    # Second line: comment line with box dimensions and timestep
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Convert LAMMPS dump file to XYZ format or sample existing XYZ file')
    parser.add_argument('input_file', help='Path to input file (LAMMPS dump or XYZ format)')
    parser.add_argument('output_file', help='Path to output XYZ file')
